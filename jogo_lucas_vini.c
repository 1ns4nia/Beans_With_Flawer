...
void gotoxy(int x, int y){
    ...
    int borda[2][6]={{205,186,201,187,200,188}, //tipos de borda, os numeros sao caracteres Ascii
                    {196,'|',218,191,192,217}};
    for(c = colunaInicial+1; c <= colunaFinal-1; c++){//printa a as bordas de cima e baixo de esquerda pra direita, usa-se coluna +1 coluna -1 pra poder deixar espa�o pras bordas
        gotoxy(c,linhaInicial); printf("%c", borda[tipo][0]);
        gotoxy(c,linhaFinal); printf("%c", borda[tipo][0]);
 }
    for(l=linhaInicial+1;l<=linhaFinal-1;l++){//printa as bordas da esquerda e direita de cima pra baixo deixando espa�o pras quinas
...
void Spawn(int qntd){ //int qntd � pego do int main, no caso 10, entao serao no maximo 10 monstros
    int x,y,cont,a=0;
